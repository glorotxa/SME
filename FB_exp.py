...
def FBexp(state, channel):
    ...
    # Negatives
    trainrn = create_random_mat(trainr.shape, np.arange(
                  state.Nright + state.Nshared))
    trainln = create_random_mat(trainr.shape, np.arange(
                  state.Nright, state.Nright + state.Nshared + state.Nleft))
    ...
    rankrfunc = RankRightFnIdx(simfn, embeddings, leftop, rightop,
            subtensorspec=state.Nright + state.Nshared)
    ...
    for epoch_count in xrange(1, state.totepochs + 1):
        ...
        if (epoch_count % state.test_all) == 0:
            ...
            out = []
            outb = []
            resvalid = RankingScoreRightIdx(rankrfunc,
                    validlidx, validridx, validoidx)
            state.valid = np.mean(resvalid)
            restrain = RankingScoreRightIdx(rankrfunc,
            ...
            if state.bestvalid == -1 or state.valid < state.bestvalid:
                restest = RankingScoreRightIdx(rankrfunc,
...
def launch(datapath='data/', dataset='FB', Nright=8309, Nshared=7785,
        Nleft=64971, Nent=81078, Nrel=13, loadmodel=False, loademb=False,
        op='Unstructured', simfn='Dot', ndim=50, nhid=50, marge=1., lremb=0.1,
        lrparam=1., nbatches=100, totepochs=2000, test_all=1, neval=50,
        seed=666, savepath='.'):
