...
def WKexp(state, channel):
    ...
    if state.shared == False or state.op == 'SE':
    ...
    trainfunc = TrainFn(simfn, embeddings, leftop, rightop,
            marge=state.marge)
    rankofunc = RankRelFnIdx(simfn, embeddings, leftop, rightop,
            subtensorspec=state.Nrel)
    ...
    for epoch_count in xrange(1, state.totepochs + 1):
        ...
        trainr = trainr[:, order]
        traino = traino[:, order]
        trainon = trainon[:, np.random.permutation(trainon.shape[1])]
        ...
        for i in range(state.nbatches):
            ...
            outtmp = trainfunc(state.lremb, state.lrparam / float(batchsize),
                    tmpl, tmpr, tmpo, tmpl, tmpr, tmpon)
        ...
        if (epoch_count % state.test_all) == 0:
            ...
            out = []
            outb = []
            resvalid = RankingScoreRelIdx(rankofunc,
                    validlidx, validridx, validoidx)
            state.valid = np.mean(resvalid)
            restrain = RankingScoreRelIdx(rankofunc,
            ...
            if state.bestvalid == -1 or state.valid < state.bestvalid:
                restest = RankingScoreRelIdx(rankofunc,
